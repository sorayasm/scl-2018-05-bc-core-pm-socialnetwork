...
function printWall(myUserId) {
    ...
    .on("child_added", (newPublicacion) => {
        ...
        publiButtonsDiv.className = "col-2 btnIcons text-right";
        publiButtonsDiv.innerHTML = `<button onclick="paintHeart('${newPublicacion.key}')"><i class="far fa-heart" id="cora-${newPublicacion.key}"></i> ${newPublicacion.val().likes}</button>
        <button onclick="edit('${newPublicacion.key}')"><i class="far fa-edit"></i></button>
...
function paintHeart(key) {
    ...
    printWall(firebase.auth().currentUser.uid);
};
